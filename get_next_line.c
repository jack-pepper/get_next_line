...
char	*get_next_line(int fd)
{
	...
	while (!found_line)
	{
		res = read_and_stash(fd, buf, &stash);
		if (res == -1)
			return (NULL);
		else if (res == 1)
...
int	read_and_stash(int fd, char *buf, char **stash)
{
	...
	//	printf("[2-1] No bytes read!\n");
		if (*stash[0] != '\0')
		{
	//		printf("We enter the no bytes loop...\n");
			return (2);
		}
		return (-1);
	}
...
char	*seize_line(char *next_line, char **stash, char *cursor)
{
	...
	//printf("We enter seize_line...\n");
	i = 0;
	if (cursor == NULL)
	{
		cursor = *stash;
	//	printf("Cursor: %p - Stash: %p\n", cursor, *stash);
		while ((*stash)[i] != '\0')
		{
			cursor++;
			i++;
